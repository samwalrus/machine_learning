%:-use_module(library('~/media/sam/9bb6ab40-5f17-481e-aba8-7bd9e4e05d66/home/sam/Documents/Prolog_practise/logical_purity.pl')).
...
thingb(As2):-
	findall(As,thinga(As),As2).

thingc(As,Features,Truths):-
	maplist(make_int,As,As2),
	length(DataNoClass,L),
	numlist(1,L,Numlist),
	maplist(idnumber_class_attributevalue_struct,Numlist,Classes,As2,Is),
...
thingd(Features,Truths):-
	thingb(As2),
	maplist(thingc,As2,Features,Truths).
